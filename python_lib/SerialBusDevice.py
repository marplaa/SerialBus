...
class SerialBusDevice:
    ...
    def __init__(self, device_addr, serialbus=None, baud = None, serialport=None, vidpid = None, serialnum = None, device_type=None):
        if serialbus is not None:
            self.serialbus = serialbus
        else:
            self.serialbus = SerialBus.SerialBus(port = serialport, baud = baud, vidpid = vidpid, serialnum = serialnum)
    ...
    def get_config(self):
        import json
        answer = self.serialbus.send_request_wait(self.device_addr, bytes([4]))
        answer_str = "";
        for char in answer:
            answer_str += (chr(char))
...
class SerialBusDeviceTCP(SerialBusDevice):
    ...
    def __init__(self, device_addr, serialbus=None, host=None, port=None, device_type=None):
        if serialbus is not None:
            ...
            if host is not None and port is not None:
                self.serialbus = SerialBus.SerialBusTCP()
                try:
                    self.serialbus.connect(host, port)
                except:
                    raise
